...
module.exports = async (title, options) => {
  ...
  if (!date || !time) {
    console.log("❌ Please provide both --date and --time");
    return;
  }

  const dateValid = dayjs(date, "YYYY-MM-DD", true).isValid();
  if (!dateValid) {
    console.log("❌ Invalid date. Use a valid date in YYYY-MM-DD format.");
    return;
  }

  const timeValid = /^([01]\d|2[0-3]):([0-5]\d)$/.test(time);
  if (!timeValid) {
    console.log("❌ Invalid time format. Use HH:mm in 24-hour format (e.g., 14:30).");
    return;
  }

  const scheduledDateTime = dayjs(`${date}T${time}`);
  if (!scheduledDateTime.isValid() || scheduledDateTime.isBefore(dayjs())) {
    console.log("❌ Cannot create a task scheduled in the past.");
    return;
  }
  ...
  try {
    ...
    console.log(`✅ Task added: "${title}" on ${date} ${time}`);
  } catch (err) {
